...
| 轨道 | 进度 |
| --- | --- |
| **A · 数据准备** | 🔶 轨迹已恢复（两遍识别 + 13 条人工指认）；**素材正在重建**（作业 1379212） |
| **B1 · 后端** | ✅ a / b / c 全部完成，测试 **149 全过** |
| **B2 · 前端** | ✅ 另一 agent 完成，已合入 `66a3d8e`；本轮补了登录补发（**未跑测试**） |
| **B3 · 测试** | 🔶 后端 149 条全过；**前端跑不了**——这台机器没有 node 工具链 |
| **C · 汇合** | ❌ 未做 |

> ⚠️ **接手第一件事**：确认作业 `1379212` 跑完，然后
> ① 重生成 `tasks_import_v3.json`（现在用编号命名，且排除 `out/dropped.txt`）
> ② 重跑 `fix_transcripts.py`
> ③ 跑 `stage_upload.py` 摊出按编号命名的上传树
>
> ⚠️ **前端有一处没验证过**：`src/storage/syncingRepository.ts` 的
> `resyncSubmitted()` 与 `tests/resync.test.ts`（12 条）是在没有 node 的机器上
> 写的，逐条手工核对过类型与签名，但**从没实跑**。有环境的地方先跑一遍。

## 本轮（2026-09-17）做完的

**B1-b · 完成判定与版本**
- `Submission` 加 `dimension`：原先效价与唤醒被并进同一条版本链，
  交完效价再交唤醒会被记成效价的修订版
- `app/completion.py`：两维齐全才算完成；读取取最新；被顶替的旧版打标记
- `/admin/progress` 新增 `in_progress` 列（动过但两维没齐的）
- `manage.py purge-superseded --keep 3`：只删多余版本的采样点，元数据保留

**编号收拢成一套**
- 新表 `sample_numbers`：`display_id` 主键、`source_id` 唯一，一一对应由
  数据库保证；`tasks.display_id` 列删除，不再有副本
- 对外一律用编号：`src` / `task_id` / `media_id` 都不再出现原始样本名
- `assign-display-ids --start 3500` 给备份池另起号段（实测 939 条 → S3500–S4438）
- 清掉 `public/tasks.json` 里 8 个占用正式号的假编号

**前端**
- 登录时补发卡在本机的提交（标完点保存恰好断网、再关掉浏览器，
  原先那条会永远丢在本机）
